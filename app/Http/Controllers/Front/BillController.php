...
class BillController extends Controller
{
    ...
    public function home()
    {
        ...
        // $response['transferes'] = $this->paymentTransaction
        //     ->whereIn('member_id', $member)
        //     ->where('is_success', 0)
        //     ->where('payment_type','=', 'transfer')
        //     ->orderBy('id', 'desc');  
            
        $response['transferes'] = DB::table('payment_transactions')
                                    ->where('member_id', '=', $this->request->member->id)
                                    ->where('deleted_at', NULL)
                                    ->where( function($query) {
                                        $query->where('payment_type','=','transfer')
                                         ->orWhere('payment_type','=','qr credit');
                                        })
                                    ->get();
            //  dd( $response['transferes']);
        return view('front.bill.home', $response);
    }
    ...
    public function getPreviewBills()
    {
        ...
        $response['data'] = [
            ...
            'transfer_url' => urldecode(url('/') . '/front/transfer/home?' . http_build_query($params)),
            'transfer_params' => [
                'version' => $version,
                'merchant_id' => $merchantId,
                'currency' => $currency,
